...
An identity can instead keep its authhead in an AuthGuard covenant, with the authority to spend it tokenized as an "AuthKey" NFT, both CashTokens Studio's conventions. The covenant address follows from the key's category, so the wallet derives it and lists it rather than being told (`utils/tools/authGuard.ts`), recognises a key by its shape and confirms it by what its covenant holds, and protects the key the way it protects an identity UTXO. The covenant's own spends belong to the tools that build them.
