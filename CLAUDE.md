...
mainnet-js configures `@electrum-cash/web-socket` to keep connections alive across visibility changes (tab switches, app backgrounding, window minimizing) rather than disconnecting/reconnecting. This matters because wallet subscriptions (balance watches, token monitors) are fire-and-forget callbacks via `runAsyncVoid`, so forcibly rejected electrum requests would surface as uncaught promise errors. Safari and iOS still kill idle WebSockets in backgrounded tabs regardless, so a connection can come back stale; mainnet-js reconnects on the actual failure, not on the visibility change.

### Electrum Trust Model
Blockchain data comes from one electrum server at a time and is not verified. `@electrum-cash/network` is a single-server client with no cluster or SPV support, so balance, history, confirmations and block height are that server's claims rather than anything the wallet checks, and they are cached to IndexedDB. The one sanity check is the tip's age on mainnet: a newest block hours old means the server stopped following the chain, and the wallet warns (`utils/wallet/serverTip.ts`).
...
A BCMR identity is an authchain: the lineage of output 0 from the authbase, whose latest unspent output 0 is the identity UTXO. Holding that UTXO is holding the identity, and metadata binds to the chain through `BCMR` publication outputs, each committing to the hash of a registry file hosted off-chain. The spec's identities are not only tokens, which is why the code branches on whether a chain carries one.

Two invariants: resolution is forward only, authbase to authhead, and an authbase is never derived backward from a UTXO, since every transaction is one. And every identity operation is the same shape, the old identity UTXO in and the new one at output 0, spent through `store.spend.spendAuthUtxo` so the reservations narrow the pool.

`utils/tools/authchainIdentity.ts` carries the chain and its operations, `stores/identitiesStore.ts` the lists, resolves and reservations. The rest of the file map, how identities reach the page, what is trusted versus verified, the limits of discovery, and what is spec versus Studio convention: `docs/bcmr-identities.md`.

### AuthGuard and AuthKeys
An identity can keep its authhead in an AuthGuard covenant instead of in a wallet, with the authority to spend it tokenized as an "AuthKey" NFT (the AuthGuard standard, which CashTokens Studio implements). A guarded identity is recognised by deriving the covenant from every category that could open it, the AuthKeys this wallet holds included, and comparing that with the authhead's locking bytecode (`utils/tools/authGuard.ts`).

An AuthKey's category is its own: the standard's genesis spends the identity's authbase and the AuthKey's in one transaction, so the two chains merge there and share every link after. A chain resolved from an AuthKey therefore ends at the identity's authhead, and the wallet lists the identity, never the AuthKey's category. An AuthKey is held back the way an identity UTXO is; the covenant's own spends belong to the tools that build them.
...
- **Never await in argument position**: `f(await g(), x)` hides the I/O behind f's name; assign it first. A leading `(await g()).filter(...)`, `Number(await g())` or `if (await g())` hides nothing and stays as it is
