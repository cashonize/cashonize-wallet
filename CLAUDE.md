...
### Chaingraph
Chaingraph is a secondary blockchain indexer next to electrum, a GraphQL (Hasura) service that allows arbitrary queries. Electrum answers about addresses the wallet knows; it cannot follow an authchain to its head, so identity resolution runs on Chaingraph. Everything else, spending, balances and the reading of the wallet's own history, stays on electrum alone. Chaingraph is new in production use, with few public instances that may not be stable or scale, so its lookups are batched and capped per wallet open, and a failure lands on the page that wanted the answer rather than failing the wallet.
