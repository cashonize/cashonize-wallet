...
New seed phrases in Cashonize are generated by libauth's `generateBip39Mnemonic`: 128 bits of entropy encoded as a standard 12-word BIP39 phrase. The entropy comes from `crypto.getRandomValues` (Web Crypto), which is backed by the operating system's kernel CSPRNG (`getrandom()` on Linux and Android, `ProcessPrng` on Windows, Fortuna on macOS), continuously seeded from physical sources such as hardware RNG instructions and interrupt timing. This is the same randomness source used for TLS and SSH keys. Libauth additionally performs defensive checks against broken RNG implementations.
