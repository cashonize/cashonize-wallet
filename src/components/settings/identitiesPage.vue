    ...
    <div v-if="mode === 'identities'" class="section">
      ...
      <div v-else class="section">
        ...
        <div v-else class="follow-head" @click="showTokenIdentities = !showTokenIdentities">
          ...
          <!-- asking what the group is for is not asking to unfold it -->
          <span @click.stop>
            <InfoPopup>
              <div style="max-width: 300px;">{{ t('identities.follow.help') }}</div>
              <div class="info-popup-note" style="max-width: 300px;">{{ t('identities.follow.settingNote') }}</div>
            </InfoPopup>
          </span>
...
/* the whole head is the group's toggle, its chevron at the end like a card header's */
.follow-head {
  display: flex;
  align-items: center;
  gap: 8px;
