...
onMounted(() => {
  ...
  if (state) {
    ...
    if (state.timestamp !== undefined) {
      ...
      autoDuration.value = Math.ceil(delta / 60000); // preload duration
    }
  }
});
      ...
      <fieldset class="dialogFieldset"> 
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem">
          ...
          <label class="radio-option">
            ...
            requests
            <span v-if="autoMode === 'count' && autoCountLeft">({{ autoCountLeft }} left)</span>
          </label>
          <label class="radio-option">
            ...
            minutes
            <span v-if="autoMode === 'time' && autoTimeLeft">({{ autoTimeLeft }} left)</span>
          </label>
