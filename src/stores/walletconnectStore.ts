...
import { NetworkType, HDWallet, SignedMessage } from "mainnet-js";
...
export const useWalletconnectStore = defineStore("walletconnectStore", () => {
  ...
  function wcSessionProposal(sessionProposal: WalletKitTypes.SessionProposal) {
    ...
    const currentNetwork = mainStore.wallet.network == NetworkType.Mainnet ? "mainnet" : "chipnet"
  ...
  async function approveSession(
    ...
    const newWcAddress = mainStore.wallet.getDepositAddress();
  ...
  async function signTransactionWC(transactionRequestWC: WalletKitTypes.SessionRequest) {
    ...
    if (wcTransactionObj.broadcast) {
      try{
        ...
        const txId = await mainStore.wallet.submitTransaction(hexToBin(signedTxObject.signedTransaction));
