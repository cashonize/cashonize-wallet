...
// Minimal loan key registry using the real parse bytecode from the ParityUSD
// loan key BCMR template. The parse bytecode checks commitment length to
// determine type ("" for empty, "01" for active loan with 10 fields).
// It uses OP_UTXOVALUE for the collateral field and multiplies interest rates
// by 365 to convert per-period rates to APR.
function createLoanKeyRegistry(categoryId: string): Registry {
  return {
    version: { major: 1, minor: 0, patch: 0 },
    latestRevision: "2025-01-01T00:00:00.000Z",
    registryIdentity: { name: "Loan Key Test Registry" },
    identities: {
      [categoryId]: {
        "2025-01-01T00:00:00.000Z": {
          name: "ParityUSD Loan Key",
          description: "For management of a ParityUSD loan.",
          token: {
            category: categoryId,
            symbol: "PUSD-LOANKEY",
            nfts: {
...
describe("End-to-end loan key parsing without extension", () => {
  const registry = createLoanKeyRegistry(loanKeyCategory);
  ...
  it("should have extension config in registry identity", () => {
    const identity =
      registry.identities![loanKeyCategory]!["2025-01-01T00:00:00.000Z"]!;
    expect(identity.extensions).toBeDefined();
    expect(identity.extensions!.parityusd).toBeDefined();
  });
});
...
describe("End-to-end loan key parsing with extension", () => {
  ...
  it("should parse as type '01' with 10 named fields after extension transplant", async () => {
    ...
    // Step 2: Parse the modified UTXO with the registry
    const result = parseNft(modifiedUtxo, registry);
