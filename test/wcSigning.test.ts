...
import { cashNinjaJsonString } from "./fixtures/wcFixtures";
...
describe('test createSignedWcTransaction', () => {
  it('should create a signed transaction from wc info', async() => {
    const requestParams = parseExtendedJson(cashNinjaJsonString) as WcTransactionObj;
    ...
    const userInput = sourceOutputs[1]
    ...
    const encodedTransaction = createSignedWcTransaction(
      transaction, sourceOutputs, signingInfo, walletLockingBytecodeHex
    );
